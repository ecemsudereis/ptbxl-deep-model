...
The ablation runner is used to compare the complete model with reduced versions of the architecture. Each run writes its metrics, configuration, and plots under `ablation/results/`.
...
The script can also generate the following artifacts:
...
The ablation set includes the full model and reduced variants where one component is removed or changed:
...
`minimal_supported` keeps the main CNN and recurrent classification path active while disabling the optional DAE, residual, attention, and VAE components. I used this version as the most reduced comparison that still fits the current model interface.
